...
;;          balls painted on it.
;; STRATEGY: Use observer template on BallList blist
...
;; ball-after-tick : Ball Racker-> Ball
...
;; STARTEGY: use observer template for ball on b
...
(begin-for-test
  ...
  (check-equal? (racket-after-tick unselected-racket-left-wall)
                ...
                "(racket-after-tick unselected-racket-left-wall)
                  returned unexpected racket")
  (check-equal? (racket-after-tick unselected-racket)
...
(define (check-position b r)
  (and
   (not(= (ball-y b) (racket-y r)))
   (not
    (and
     (> (ball-x b) (+ (racket-x r) RACKET-HALF-WIDTH))
     (< (ball-x b) (- (racket-x r) RACKET-HALF-WIDTH))))))

...
(define (is-in? element list2)
  (ormap (; Int -> Boolean
          ; GIVEN: an integer n
          ; RETURNS: true iff its argument is equal to element
          lambda (n) (= element n)) list2))
...
;; STRATEGY: check for condition and use HOF andmap on IntList
  ...
  (cond
    [(= (length list1) (length list2))
     (andmap
      (; Int -> Boolean
       ; GIVEN: an Integer x
       ; RETURNS: true iff its argument x is in list2
       lambda (x) (is-in? x list2))
      list1)]
    [else
     #false]))
...
(define (permutations l)
  (cond
    ...
    [else
     (sort-permutations(permutation l))]))

;; permutations-start : IntList -> IntListList
...
;;           (list 3 1 2) (list 2 1 3)
;;           (list 1 3 2) (list 1 2 3))
;; STRATEGY: use HOF foldr, and map on l
(define (permutation l)
  (foldr append empty (map (; Integer -> IntList
                            ; GIVENT: an Integer n
                            ; RETURNS: an IntList with argument n as the first  
                            ;          and the permutation of rest as the 
                            ;          rest of IntList.
                            lambda (n) (map (; IntList -> IntList
                                             ; GIVEN : an IntList nl
                                             ; RETURNS : an IntList with 
                                             ;           the argument nl
                                             ;           as the rest of the
                                             ;           list.
                                             lambda (nl) (cons n nl))
                                            (permutations (remove n l))))l)))  
...
(begin-for-test
  ...
  (check-equal? (sort-insert (list 1 3 2) (list (list 1 2 3) (list 2 1 3)))
                (list (list 1 2 3) (list 1 3 2) (list 2 1 3))
                "(sort-insert (list 1 3 2) (list (list 1 2 3) (list 2 1 3)))
